...
class Report(models.Model):
    ...
    patient_social_security_number = fields.Char(related='admission_id.patient_social_security_number',
                                                 string='Social Security', readonly=True)
    doctor_id = fields.Many2one('hospital.doctor', string='Doctor', related='admission_id.doctor_admissions_id',
                                readonly=True)
    bed_id = fields.Many2one('hospital.bed', string='Bed', related='admission_id.bed_id', readonly=True)
    admission_date = fields.Datetime(string='Admission Date', related='admission_id.admission_date', readonly=True)
    discharge_date = fields.Datetime(string='Discharge Date', related='admission_id.discharge_date', readonly=True)
